    ...
    try:
        ...
        for i in range(start, end):
            ...
            if bull_cand:
                ...
                zone_quality = float(max(0.0, min(1.0, 0.5 * disp_score + 0.3 * vol_score + 0.2 * fresh_score)))
                bull_zones.append({"type": "BULLISH_DEMAND", "dist": float(dist), "ref": float(zone_high), "i": i, "quality": zone_quality})
            if bear_cand:
                zone_low = min(o, c)
                zone_high = h
                dist = 0.0 if zone_low - prox <= ep <= zone_high + prox else min(abs(ep - zone_low), abs(ep - zone_high))
                ...
                fresh_score = float(max(0.0, 1.0 - 0.25 * max(0, touch_count - 1)))
                zone_quality = float(max(0.0, min(1.0, 0.5 * disp_score + 0.3 * vol_score + 0.2 * fresh_score)))
                bear_zones.append({"type": "BEARISH_SUPPLY", "dist": float(dist), "ref": float(zone_low), "i": i, "quality": zone_quality})
    ...
    *,
    ...
    if stage == "PRE" and weak_tape_env:
        scalp_weak_tape_diag = _assess_scalp_weak_tape_turn(
            df,
            direction=str(bias),
            trigger_line=float(ref_vwap) if isinstance(ref_vwap, (float, int)) else float(scalp_accept_line) if isinstance(scalp_accept_line, (float, int)) else None,
            atr_last=float(atr_last) if atr_last is not None else None,
        )
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    if accept_components:
        ...
        if valid_keys:
            accept_src = max(valid_keys, key=lambda k: accept_component_weights.get(k, 0.0))
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        if direction == "LONG":
            ...
            stale_breakout_local = bool(last_price > break_trigger_local + 0.60 * atr_last + entry_pad and dist_pb_band_local > prox_dist_local)
        else:
            dist_pb_band_local = 0.0 if (last_price <= pb2_local and last_price >= pb1_local) else min(abs(last_price - pb2_local), abs(last_price - pb1_local))
            stale_breakout_local = bool(last_price < break_trigger_local - 0.60 * atr_last - entry_pad and dist_pb_band_local > prox_dist_local)
        ...
        breakout_extension_state_local = {"penalty": 0.0, "extended": False, "exhausted": False, "dist_accept_atr": 0.0, "dist_vwap_atr": 0.0, "momentum_fade": False, "stalling": False}
        if actionable_local:
            ...
            if near_pullback_local and not near_break_local:
        ...
        return {
            ...
            "breakout_bias_score": int(breakout_bias_score_local) if actionable_local else 0,
            "structure_phase": structure_phase_local if actionable_local else "UNSET",
            ...
            "prox_atr_local": float(prox_atr_local) if actionable_local else 0.0,
        }
